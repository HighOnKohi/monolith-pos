...
import { KeyRound, ShieldAlert, ArrowRight, UserCheck, Delete, Loader2, UtensilsCrossed } from 'lucide-react'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4 animate-fade-in overflow-y-auto">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden my-auto">
        ...
        <div className="bg-[#14274E] px-8 pt-8 pb-7 text-center relative overflow-hidden">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 border border-white/15 text-[#E9C46A] text-[10px] font-black uppercase tracking-wider mb-2">
            <UtensilsCrossed className="w-3 h-3" />
            <span>Service &amp; Floor Ordering</span>
          </div>

          <h2 className="text-xl font-black text-white tracking-tight">Service Operational Access</h2>
          <p className="text-xs font-semibold text-slate-300 mt-1 max-w-xs mx-auto">
            Please verify your Staff ID to begin your service shift and take table orders.
          </p>
        </div>
        ...
        <div className="p-6 sm:p-8 space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="service-staff-id-input" className="block text-xs font-black text-slate-600 uppercase tracking-wider mb-2">
                Enter Server / Staff ID
              </label>
          ...
          <div className="pt-2 border-t border-slate-100">
            <p className="text-[11px] font-bold text-slate-400 mb-2 flex items-center gap-1.5">
              ...
              <span>Quick Select Authorized Server / Staff:</span>
            </p>
            <div className="flex flex-wrap gap-1.5">
              <button
                type="button"
                onClick={() => handleQuickSelect(1005)}
                className="px-2.5 py-1 bg-purple-50 hover:bg-purple-100 border border-purple-200 rounded-lg text-[11px] font-bold text-purple-800 transition-colors cursor-pointer"
              >
                1005 — Elena (Staff)
              </button>
              <button
