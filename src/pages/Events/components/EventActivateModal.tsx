        ...
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 text-xs space-y-2">
          <div className="flex items-center justify-between text-slate-600">
            ...
            <span className="font-bold text-[#14274E]">
              {event.presetId
                ? `${layoutPresets.find((p) => p.LAYOUT_PRESET_ID === event.presetId)?.PRESET_NAME ?? `Preset #${event.presetId}`} (${layoutPresets.find((p) => p.LAYOUT_PRESET_ID === event.presetId)?.MAX_PAX ?? 50} Pax capacity)`
                : 'None (Default Floor Plan)'}
            </span>
          </div>
          <div className="flex items-center justify-between text-slate-600">
            <span className="flex items-center gap-1.5 font-medium">
